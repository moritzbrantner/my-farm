...
export function App() {
  ...
  return (
    <main className="app">
      <FarmScene view={view} selection={selection} onSelect={setSelection} />
      ...
      <aside className="side-panel">
        ...
        <SelectionPanel
          ...
          send={send}
        />
        <Orders catalog={catalog} view={view} send={send} />
      </aside>
      <BuildTray catalog={catalog} view={view} send={send} />
    </main>
  );
}
      ...
      {buildKinds.map((kind) => {
        ...
        return (
          <button
            type="button"
            key={kind}
            disabled={built.has(kind) || view.level < unlockLevel}
            onClick={() =>
              send({
                type: "buy_structure",
                structure_kind: kind,
