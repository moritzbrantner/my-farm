...
function MovingCar({
  ...
  return (
    ...
    >
      ...
      {marked ? <mesh position={[0, 0.54, 0]} visible={false}><sphereGeometry args={[0.01, 4, 4]} /></mesh> : null}
    </group>
  );
}
...
function StatusProgressBar({
  ...
  return (
    <group position={[0, 0.9, -footprint.height * 0.12]}>
      <group>
        <mesh>
...
function StructureLabel({
  ...
  return (
    <group position={[0, labelY, -footprint.height * 0.08]}>
      <mesh>
        <boxGeometry args={[Math.min(1.2, Math.max(0.52, label.length * 0.08)), 0.12, 0.035]} />
        <meshBasicMaterial color="#fff7d0" transparent opacity={0.82} depthWrite={false} />
      </mesh>
    </group>
  );
}
